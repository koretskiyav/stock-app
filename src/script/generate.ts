...
function getType(value: any): "string" | "number" | "boolean" {
...
function generateMeta(fileName: string, data: any[]) {
    ...
    // Raw Interface
    const rawInterface = `interface ${rawName} {\n` + 
        keys.map(k => `  "${k}": string;`).join("\n") + 
        "\n}";

    // Clean Interface
    const cleanInterface = `export interface ${paramsName} {\n` + 
        keys.map(k => `  ${_.camelCase(k)}: ${schema[k]};`).join("\n") + 
        "\n}";

    // Mapper
    const mapper = `export const ${_.camelCase(paramsName)}: ${paramsName}[] = (data as ${rawName}[]).map(item => ({\n` +
        keys.map(k => {
